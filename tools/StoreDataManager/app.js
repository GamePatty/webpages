...
function readArray(container, isNumber) {
    ...
    return values.length ? values : -4.0;
}
...
function addProduct(data = {}) {
    ...
    inputs[0].value = data.StoreProductType || "Item";
    inputs[1].value = data.ItemID || uniqueItemID("item");
    inputs[2].value = data.ProductID || "";
    inputs[3].value = data.Title || "";
    inputs[4].value = data.Desc || "";
    inputs[5].value = data.Price || 0;

    inputs[1].oninput = refreshOfferDropdowns;
    ...
    dup.onclick = () => {
        addProduct({
            StoreProductType: inputs[0].value,
            ItemID: uniqueItemID(inputs[1].value),
            ProductID: inputs[2].value,
...
function toggleOffer(sel) {
    const box = sel.closest(".offer");
    const isManual = sel.value === "Manual";

    box.querySelector(".manual").style.display = isManual ? "block" : "none";
    box.querySelectorAll(".time").forEach(r => {
        r.style.display = isManual ? "none" : "block";
    });
}
...
function exportJSON() {
    ...
    productsDiv.querySelectorAll(".product").forEach(p => {
        ...
        if (i[0].value === "Item") {
            result.items[obj.ItemID] = obj;
        } else {
            result.bundles[obj.ItemID] = obj;
        }
    });
    ...
    offersDiv.querySelectorAll(".offer").forEach(o => {
        ...
        const start = {
            day: Number(s[4].value),
            month: Number(s[5].value),
            year: Number(s[6].value),
            hour: 0,
            minute: 0,
            second: 0
        };
        ...
        const end = {
            day: Number(s[7].value),
            month: Number(s[8].value),
            year: Number(s[9].value),
            hour: 0,
            minute: 0,
            second: 0
        };

        result.offers[s[0].value] = {
            itemID: s[0].value,
